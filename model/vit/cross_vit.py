...
class ImageEmbedder(nn.Module):  # 图像编码层
    def __init__(
            self,
            *,
            dim,  # I want to need dim
        ...
        num_patches = (image_size // patch_size) ** 2  # example patch_size=16  =>256//16 **2
    ...
    def forward(self, img):
        x = self.to_patch_embedding(img)  # image to patch
        b, n, _ = x.shape
...
class CrossViT(nn.Module):
    ...
    def forward(self, img1, img2):  # 修改成双输入
        sm_tokens = self.sm_image_embedder(img1)  # 不同尺度编码
